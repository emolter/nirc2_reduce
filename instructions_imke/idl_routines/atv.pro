...
;           [,/align] [,/stretch] [,header = header] [,/quit]
;
; REQUIRED INPUTS:
...
;
;       4-July-2002: H.Roe added /quit option
;
;       26-April-2002: H.Roe added capability to handle 3-d stacks
;                      of images.
;                      Also, added auto-zooming of images when opened.
;
;       This version is 1.3HR, last modified by Aaron Barth on 
;       28 November 2000, and modified 26-April-2002 by hroe.
...
state = {                   $
          version: '1.3HR', $              ; version # of this release
...
curimnum_base = widget_base(base, /base_align_left, column = 3, frame = 1)
...
state.curimnum_text_id = cw_field(curimnum_base, $
...
state.curimnum_slidebar_id = widget_slider(curimnum_base, $
                            /drag, $ 
                            max = 1, $
                            min = 0, $
                            scr_xsize = 290L, $
...
mode_droplist_id = widget_droplist(curimnum_base, $
...
case uvalue of
    ...
    'curimnum_text':begin    ; text entry in 'im #=' box
...
if ( (n_params() EQ 0) AND (xregistered('atv', /noshow)) AND $
           (NOT keyword_set(quit))) then begin
...
;shutdown if /quit was selected
if keyword_set(quit) then atv_shutdown
